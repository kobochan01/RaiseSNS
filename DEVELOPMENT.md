...
| 2026-09-10 | #30 | 標準出力への構造化JSONログ出力を追加。logstash-logback-encoder 8.1（Jackson 3化前の最終版、Boot 3.5.0のLogback 1.5.18と整合するバージョンを選定）を導入し、リクエスト単位のtraceId（MDC、レスポンスヘッダー`X-Request-Id`）とDatadog Unified Service Tagging（service/env/version）を付与。`GlobalExceptionHandler`に想定外例外のcatch-all（500）を追加しログに必ず捕捉されるようにした。Datadog Agent自体の接続（APIキー設定等）は本Issueのスコープ外。あわせて運用監視・障害対応の設計書`docs/observability.md`を新設 |
